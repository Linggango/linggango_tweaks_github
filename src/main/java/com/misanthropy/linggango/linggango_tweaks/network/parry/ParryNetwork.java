package com.misanthropy.linggango.linggango_tweaks.network.parry;
...
public class ParryNetwork {
    ...
    public static final SimpleChannel CHANNEL = NetworkRegistry.newSimpleChannel(
            ResourceLocation.fromNamespaceAndPath("linggango_tweaks", "parry"),
            () -> PROTOCOL_VERSION,
    ...
    public static class C2SParryPacket {
        ...
        public void handle(@NonNull Supplier<NetworkEvent.Context> supplier) {
            ...
            context.enqueueWork(() -> {
                ...
                if (player != null) {
                    ParryServerHandler.onClientParryPacket(player);
                }
            });
    ...
    public static class S2CParryStartPacket {
        ...
        public void handle(@NonNull Supplier<NetworkEvent.Context> supplier) {
            ...
            context.enqueueWork(() -> DistExecutor.unsafeRunWhenOn(Dist.CLIENT, () -> () ->
                    com.misanthropy.linggango.linggango_tweaks.client.parry.ParryEffects.triggerParryStartForOther(this.entityId)
            ));
            context.setPacketHandled(true);
        }
    }
    ...
    public static class S2CParrySuccessPacket {
        ...
        public void handle(@NonNull Supplier<NetworkEvent.Context> supplier) {
            NetworkEvent.Context context = supplier.get();
            context.enqueueWork(() -> DistExecutor.unsafeRunWhenOn(Dist.CLIENT, () -> () ->
                    com.misanthropy.linggango.linggango_tweaks.client.parry.ParryEffects.triggerSuccessfulParry(this.entityId, this.tier, this.comboStage)
            ));
            context.setPacketHandled(true);
        }
    }
    ...
    public static class C2SResetComboPacket {
        ...
        public void handle(@NonNull Supplier<NetworkEvent.Context> supplier) {
            ...
            context.enqueueWork(() -> {
                ServerPlayer player = context.getSender();
                if (player != null) {
                    ParryServerHandler.resetCombo(player.getUUID());
                }
            });
            context.setPacketHandled(true);
        }
