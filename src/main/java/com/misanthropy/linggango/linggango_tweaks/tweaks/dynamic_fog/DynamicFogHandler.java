package com.misanthropy.linggango.linggango_tweaks.tweaks.dynamic_fog;
...
public class DynamicFogHandler {
    ...
    public static int biomeToleranceTicks = 60;
    public static double fogTransitionSpeed = 0.015;
    ...
    private static final String KEY_BIOME_TOLERANCE = "biomeToleranceTicks";
    private static final String KEY_FOG_SPEED = "fogTransitionSpeed";
    ...
    private static ResourceLocation stableBiomeLoc = null;
    private static ResourceLocation pendingBiomeLoc = null;
    private static long pendingBiomeStartTick = -1;
    private static boolean isBiomeStable = true;
    private static long lastToleranceUpdateTick = -1;
    ...
    public static void load() {
        try {
            ...
            if (Files.exists(path)) {
                ...
                biomeToleranceTicks = (int) linggango_tweaks$parseDouble(props, KEY_BIOME_TOLERANCE, 60.0D, 0.0D, 600.0D);
                fogTransitionSpeed = linggango_tweaks$parseDouble(props, KEY_FOG_SPEED, 0.015D, 0.001D, 0.1D);
            }
    ...
    public static void save() {
        try {
            ...
            props.setProperty(KEY_BIOME_TOLERANCE, String.valueOf(biomeToleranceTicks));
            props.setProperty(KEY_FOG_SPEED, String.valueOf(fogTransitionSpeed));
    ...
    public static void onRenderFog(ViewportEvent.@NonNull RenderFog event) {
        ...
        AtmosphereConfigManager.AtmosphereSettings centerSettings = stableBiomeLoc != null ? AtmosphereConfigManager.ATMOSPHERES.get(stableBiomeLoc.toString()) : null;
        ...
        float distLerp = isEditing ? 1.0F : (float) fogTransitionSpeed;
    ...
    public static void onComputeFogColor(ViewportEvent.@NonNull ComputeFogColor event) {
        ...
        boolean isEditing = mc.screen instanceof AtmosphereEditorScreen;
        float colorLerp = isEditing ? 1.0F : (float) fogTransitionSpeed;
    ...
    private static void linggango_tweaks$updateBiomeTolerance(Level level, Vec3 cameraPos) {
        long gameTime = level.getGameTime();
        if (gameTime == lastToleranceUpdateTick) return;
        lastToleranceUpdateTick = gameTime;

        ResourceLocation currentBiomeLoc = level.getBiome(BlockPos.containing(cameraPos)).unwrapKey().map(ResourceKey::location).orElse(null);

        if (currentBiomeLoc == null) {
            stableBiomeLoc = null;
            pendingBiomeLoc = null;
            isBiomeStable = true;
            return;
        }

        if (stableBiomeLoc == null) {
            stableBiomeLoc = currentBiomeLoc;
            pendingBiomeLoc = currentBiomeLoc;
            pendingBiomeStartTick = gameTime;
            isBiomeStable = true;
            return;
        }
        if (currentBiomeLoc.equals(stableBiomeLoc)) {
            pendingBiomeLoc = currentBiomeLoc;
            isBiomeStable = true;
        }
        else if (!currentBiomeLoc.equals(pendingBiomeLoc)) {
            pendingBiomeLoc = currentBiomeLoc;
            pendingBiomeStartTick = gameTime;
            isBiomeStable = false;
        }

        if (!isBiomeStable && (gameTime - pendingBiomeStartTick >= biomeToleranceTicks)) {
            stableBiomeLoc = pendingBiomeLoc;
            isBiomeStable = true;
        }
    }
    ...
    private static void linggango_tweaks$updateColorSampling(@NonNull Minecraft mc, @NonNull Vec3 cameraPos, float partialTick) {
        ...
        var level = mc.level;
        long gameTime = level.getGameTime();
        linggango_tweaks$updateBiomeTolerance(level, cameraPos);
        ...
        if (totalWeight > 0.0F) {
            if (isBiomeStable || stableBiomeLoc == null) {
