...
public class AntiCheeseHandler {
    ...
    private static final Predicate<Mob> HOSTILE_FILTER = entity -> entity instanceof Enemy && entity.isAlive();

    private static final Component WARNING_MESSAGE = Component.literal("§6Standing still too long... Desperation approaches.");
    private static final Component PENALTY_MESSAGE = Component.literal("§cYou are overcome by Desperation! Move!");
    ...
    public static void onPlayerTick(TickEvent.@NonNull PlayerTickEvent event) {
        ...
        if (player.tickCount % CHECK_INTERVAL == 0) {
            ...
            PlayerPositionData data = TRACKER.computeIfAbsent(uuid, k -> new PlayerPositionData(currentPos));
            double distance = currentPos.distanceTo(data.lastPos);
            boolean hostilesNearby = !player.level().getEntitiesOfClass(Mob.class, player.getBoundingBox().inflate(24.0D), HOSTILE_FILTER).isEmpty();
    ...
    private static void applyDesperationLogic(@NonNull ServerPlayer player, @NonNull PlayerPositionData data) {
        if (data.stationaryTime >= PENALTY_TIME) {
            ...
            if (player.tickCount % 40 == 0) {
                player.displayClientMessage(PENALTY_MESSAGE, true);
            }
